...
2026-09-23 v11 (Claude, Mustafa talimatı, Codex bağımsız ikinci yeniden denetim bulguları): N01–N07 P1 ve U04–U07/N08 P2 bulguları fazlar hâlinde kapsam kuralı düzeyinde düzeltildi; Codex'in 126 denemesi kalıcı HTTP/DB testine çevrildi (41 kırmızı → 126/126), matris oracle'ı sıkılaştırıldı, 32 kendi probe'u eklendi (16'sı v10 kodunda başarısızdı); 762 backend/22 golden/25 istemci testi geçti, push yapılmadı.

2026-09-23 v12 (Claude, Mustafa talimatı, Codex bağımsız üçüncü yeniden denetim bulguları): V01–V03 P1 ve V04/U08/U09 P2 bulguları faz faz düzeltildi; Codex'in 109 denemesi kalıcı HTTP/DB testine çevrildi (39 kırmızı → 109/109, U10 belgeli güvenli netleştirme olarak), 21 kendi probe'u eklendi (8'i v11 kodunda başarısızdı); tam koşudaki 512 MB OOM'un kök nedeni (FastAPI endpoint önbelleğinde app closure'ı) ölçülüp düzeltildi; 892 backend/22 golden/25 istemci testi geçti, push yapılmadı.

2026-09-23 v13 (Claude, Mustafa talimatı, Codex bağımsız dördüncü yeniden denetim bulguları): W01–W03 P1 ve W04/UX11–UX13/OBS01 bulguları faz faz düzeltildi (fiyat sınırının tutar biçiminden bağımsız tanınması, onayın ifadesinin tamamıyla değerlendirilmesi, genel alternatif dalında adsız kaynak kuralı ve yalnız kayıtlı alternatif adayı); Codex'in 91 denemesi kalıcı HTTP/DB testine çevrildi (24 kırmızı → 91/91, `safe(...)` kullanılmadan), 32 kendi probe'u eklendi (24'ü v12 kodunda başarısızdı); 1015 backend/22 golden/25 istemci testi geçti, push yapılmadı.
